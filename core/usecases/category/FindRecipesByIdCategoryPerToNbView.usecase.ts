...
export default class FindRecipesByIdCategoryPerToNbViewUsecase {
  ...
  private checkBusinessRules = (id: any, user: Token): void => {
    if (isAdmin(user)) {
      if (id) {
        if (!this.categoryRepository.existById(id)) throw new BusinessException("Cette catégorie n'existe pas");

...
export default class FindAllCommentairesByIdUserUsecase {
  constructor(private commentaireRepository: CommentaireRepository, private userRepository: UserRepository) {}

  execute = async (pseudo: any, token?: Token): Promise<Commentaire[]> => {
  ...
  private checkBusinessRules = async (pseudo: any, token?: Token): Promise<void> => {
    if(token && isLogin(token)) {
      if(pseudo) {
        if(await this.userRepository.existByPseudo(pseudo)) {
          if(pseudo !== token.pseudo) throw new BusinessException("La personne connectée n'est pas la personne correspondant au pseudo en question");
    ...
    } else throw new TechnicalException("Vous n'avez pas accès à ces ressources");
  };
}

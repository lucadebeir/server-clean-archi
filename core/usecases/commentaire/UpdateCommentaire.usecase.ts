...
export default class UpdateCommentaireUseCase {
  ...
  private checkBusinessRules = async (commentaire: Commentaire, token?: Token): Promise<void> => {
          ...
          if (token.pseudo === commentaire.pseudo) {
            if (commentaire.id_recipe) {
              if (await this.recipeRepository.existById(commentaire.id_recipe)) {
                if (commentaire.id) {
                  if (await this.commentaireRepository.existById(commentaire.id)) {
                  ...
                  } else throw new BusinessException("Le commentaire n'existe pas");
    ...
    } else throw new TechnicalException("Vous ne pouvez pas modifier cette ressource");
  };
}

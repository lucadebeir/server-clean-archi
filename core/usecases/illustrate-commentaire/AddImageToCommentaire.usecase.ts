...
export default class AddImageToCommentaireUseCase {
  ...
  execute = async (illustrateCommentaire: IllustrateCommentaire, token?: Token): Promise<string> => {
    await this.checkBusinessRules(illustrateCommentaire, token);
    return this.illustrateCommentaireRepository.addToCommentaire(illustrateCommentaire);
  };
    ...
    if (token && isLogin(token)) {
      if (illustrateCommentaire) {
        ...
        if (await this.illustrateCommentaireRepository.check(illustrateCommentaire)) {
          throw new BusinessException("Cette image est déjà associé à ce commentaire");
        }
      } else {

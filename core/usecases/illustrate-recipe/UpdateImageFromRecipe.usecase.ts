...
export default class UpdateImageFromRecipeUseCase {
  ...
  async execute(illustrateRecipe: IllustrateRecipe, token?: Token): Promise<string> {
  ...
  private async checkBusinessRules(illustrateRecipe: IllustrateRecipe, token?: Token): Promise<void> {
    if (token && isAdmin(token)) {
      if (illustrateRecipe) {
        ...
        if (!await this.illustrateRecipeRepository.check(illustrateRecipe)) {
          throw new BusinessException("Cette image n'existe pas dans cette recette");
        }
      } else {

...
export default class UpdateMenuByIdUseCase {
  constructor(private menuRepository: MenuRepository, private recipeRepository: RecipeRepository) {}
  ...
  private checkBusinessRules = async (menu: Menu, token?: Token): Promise<void> => {
    if (token && isAdmin(token)) {
      if (menu.id) {
        if (await this.menuRepository.existById(menu.id)) {
          ...
          throw new BusinessException("L'identifiant de menu " + menu.id + " ne correspond à aucune ressource existante");
        }
      } else {
